...
<Steps>
  ...
  <Step title="Log conversions (iOS only)">
    ...
    Radar.logConversion('card_setup_viewed').then((result) => {
  ...
  <Step title="Set up deep linking (iOS only)">
    Radar campaign notifications support deep linking into a specific screen in your app when a user taps a notification. This requires additional native setup in your `AppDelegate.mm`. See the [deep linking docs](https://docs.radar.com/geofencing/campaigns#deep-linking-ios-only) for setup instructions.
  </Step>
</Steps>
