...
<Steps>
  ...
  <Step title="Configure remote tracking options">
    In the Radar dashboard, navigate to the SDK configuration section in [settings](https://dashboard.radar.com/settings#sdk-configuration). Toggle on "Remotely set tracking options". Then toggle on "Enable on-trip tracking options" and set the "Tracking preset" to "Continuous". Toggle on "Enable in-geofence tracking options", set the "Tracking preset" to "Custom" and replace the JSON blob below with the following tracking options on iOS and Android respectively:
