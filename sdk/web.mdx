...
| `networkTimeoutInterval` | `10` | number | The number of seconds to wait for a network request before an error is thrown. Values are clamped to `[1, 300]` seconds. |
| `trackVerifiedAutoFailover` | `false` | `true`, `false` | Determines whether `Radar.trackVerified()` should automatically retry on the secondary verified host when the primary verified host returns a non-Radar response. Requires `@radarlabs/plugin-fraud`. |
...
| `debug` | `false` | `true`, `false` | Determines whether to include the raw API response body on results, as `response`. Defaults to `true` when using a test publishable key. |
...
Every API response includes an `X-Radar-Request-ID` header with a unique identifier for the request. The Web SDK surfaces this identifier as `meta.requestId` on the API response body.
...
Radar.trackOnce()
  .catch((err) => {
    const requestId = err.response?.meta?.requestId;
  });
```

When the SDK is initialized with the `debug` option enabled, the API response body is also included on successful results, as `response`:

```javascript
Radar.initialize('prj_live_pk_...', { debug: true });

Radar.trackOnce()
  .then((result) => {
    const requestId = result.response?.meta?.requestId;
  });
```
