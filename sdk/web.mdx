...
| `networkTimeoutInterval` | `10` | number | The number of seconds to wait for a network request before an error is thrown. |
| `trackVerifiedAutoFailover` | `false` | `true`, `false` | Determines whether `Radar.trackVerified()` should automatically retry on the secondary verified host when the primary verified host fails. Requires `@radarlabs/plugin-fraud`. |
