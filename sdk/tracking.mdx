...
- **`syncLocations`**: Determines which location updates to sync to the server. `.all` syncs all location updates to the server, `.stopsAndExits` syncs only stops and exits to the server, `.events` only syncs location updates that will generate an event, `.none` syncs no location updates to the server. Defaults to `.all`.

  When using `.events`, the SDK only syncs location updates that would generate one of the events below. Each product (Geofences, Places, or Beacons) must be enabled *and* the corresponding event types must be selected in your Radar dashboard settings (e.g., Settings > Geofences > Event types) for those locations to sync:
    - Geofences: `user.entered_geofence`, `user.exited_geofence`, `user.dwelled_in_geofence`
    - Places: `user.entered_place`, `user.exited_place`
    - Beacons: `user.entered_beacon`, `user.exited_beacon`
...
- **`sync`**: Determines which location updates to sync to the server. `ALL` syncs all location updates to the server, `STOPS_AND_EXITS` syncs only stops and exits to the server, `EVENTS` only syncs location updates that will generate an event, `NONE` syncs no location updates to the server. Defaults to `STOPS_AND_EXITS`.
