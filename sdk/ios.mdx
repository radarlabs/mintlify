...
<Info>
  Only select `RadarSDK` unless you are also intending to use the location metadata feature. Refer to [**this section**](https://docs.radar.com/sdk/ios#location-metadata) for more information.
</Info>
...
If you are planning to use `RadarTrackingOptions.presetContinuous` or `RadarTrackingOptions.presetResponsive`, or if you are planning to [range beacons by UUID only](https://docs.radar.com/beacons#ranging-by-uuid-only), you must enable the _Location updates_ and _Background fetch_ background modes.
...
Note that this requires additional justification during App Store review. Learn more [below](https://docs.radar.com/sdk/ios#submit-to-app-store).
...
- `.errorRateLimit`: too many requests ([rate limit](https://docs.radar.com/api#track) exceeded)
...
<Info>
  Though we recommend using presets for most use cases, you can modify the presets. See the [**tracking options reference**](https://docs.radar.com/sdk/tracking).
</Info>
...
Learn more about starting, completing, and canceling trips in the [trip tracking documentation](https://docs.radar.com/trip-tracking).
...
## **Listening for events with a delegate**[**#**](https://docs.radar.com/sdk/ios#listening-for-events-with-a-delegate)
...
## **Manual tracking**[**#**](https://docs.radar.com/sdk/ios#manual-tracking)
...
If the user has granted location permissions, you can range and monitor [beacons](https://docs.radar.com/beacons).
...
With the [context API](https://docs.radar.com/api#context), get context for a location without sending device or user identifiers to the server:
...
With the [forward geocoding API](https://docs.radar.com/api#forward-geocode), geocode an address, converting address to coordinates:
...
With the [reverse geocoding API](https://docs.radar.com/api#reverse-geocode), reverse geocode a location, converting coordinates to address:
...
With the [IP geocoding API](https://docs.radar.com/api#ip-geocode), geocode the device's current IP address, converting IP address to city, state, and country:
...
With the [autocomplete API](https://docs.radar.com/api#autocomplete), autocomplete partial addresses and place names, sorted by relevance:
...
With the [geofence search API](https://docs.radar.com/api#search-geofences), search for geofences near a location, sorted by distance:
...
With the [places search API](https://docs.radar.com/api#search-places), search for places near a location, sorted by distance:
...
With the [address validation API](https://docs.radar.com/api##validate-an-address) (currently in beta), validate a structured address in the US or Canada:
...
With the [distance API](https://docs.radar.com/api#distance), calculate the travel distance and duration between an origin and a destination:
...
With the [matrix API](https://docs.radar.com/api#matrix), calculate the travel distance and duration between multiple origins and destinations for up to 25 routes:
...
With the [conversions API](https://docs.radar.com/api#log-a-conversion), log a conversion, such as a purchase or signup, to analyze alongside your app's location activity:
