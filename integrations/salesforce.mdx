...
![Salesforce Verify Integration Gi](/images/salesforce-verify-integration.gif)

Additionally, check the Radar data extension in Salesforce's _Contact Builder_ to verify records are delivering.

All integration delivery can be monitored via the integration's event logs by clicking _View event logs_ on the [Integrations page](https://dashboard.radar.com/integrations#salesforce).
