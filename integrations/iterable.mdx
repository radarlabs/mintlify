...
<Note>
  If your Iterable user ID differs from your Radar user ID, you can optionally set `metadata['iterable:user_id']` to specify the user ID sent to Iterable. When set, this value will be used as the `userId` in the Iterable integration instead of the Radar user ID.
</Note>
