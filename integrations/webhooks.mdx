...
Create a webhook on the Radar [Integrations page](https://dashboard.radar.com/integrations) under _Webhooks_. For _Environment_, choose _Test_ to receive callbacks for events in the _Test_ environment, or choose _Live_ to receive callbacks for events in the _Live_ environment. For _Event Delivery_, choose _Single Event_ to receive a single event in each request, or choose _Multiple Events_ to receive multiple events in each request when events occur simultaneously. Finally, enter a URL.
