...
User insights turn a user's location history into current context, including whether the user is at home, at work, traveling, or commuting.

Radar infers likely home and work locations from recurring stopped-location patterns. As new location updates arrive, Radar compares the user's current location with those inferred locations and returns intelligence about the user's location.
...
Radar generates events when a user's location update causes insight state to change:
