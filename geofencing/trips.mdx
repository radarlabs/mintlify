...
When using legs, set `destinationGeofenceTag` and `destinationGeofenceExternalId` on each leg rather than on the trip options directly. The first leg is automatically set to `started` when the trip begins.
