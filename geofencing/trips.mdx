...
With silent push notifications enabled, we will attempt to initiate track requests on users that has lost tracking updates. 
...
If the trip's last update time is older than the `stale_threshold` 
