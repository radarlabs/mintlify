...
The title, body, and button text can all be customized for each user, geofence, and place that triggers the notification or in-app-message. For example, a notification title of "Hello! {{ user.userId }}" will be send as "Hello! Alice" and "Hello! Bob" for user Alice or Bob

`user` will be available for all campaigns, `geofence` will be available for geofence targeting campaigns and `place` will be available for place targeting campaign.
