...
First, create a public-private key pair. Save your private key. You private key should never leave your server. Copy your public key. For example:
...
On the [Settings page](https://dashboard.radar.com/settings), under *Auth tokens*, add a new public key. Paste the full contents of your public key PEM file, including the `BEGIN` and `END` lines.

An auth token is a JSON Web Token (JWT). To create one, sign a JWT using ES256 on your server. The payload should include `now` for `iat`, `now + ttl` for `exp`, `radar-api-v1` for `aud` (audience), a custom value for `iss` (issuer), the public key *Key ID* from the dashboard for `kid`, and any optional constraints as claims. For example:
