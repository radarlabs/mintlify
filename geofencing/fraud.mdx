...
From there, install the RadarFraudSDK using either the [Swift Package](https://github.com/radarlabs/radar-sdk-ios-fraud-spm) or [CocoaPod](https://cocoapods.org/pods/RadarSDK), [integrate the SDK](/sdk/sdk), complete the steps below, and call `Radar.trackVerified()` or `Radar.startTrackingVerified()`. Radar will perform fraud and jurisdiction checks as described below.
