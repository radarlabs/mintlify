...
As an alternative to publishable API keys, you can create temporary auth tokens. Auth tokens have a `ttl` and optional constraints as claims (e.g., `product`, `expectedCountryCode`, `expectedStateCode`, and/or `userId).

To create auth tokens, first create a public-private key pair. Save your private key and ensure that it never leaves your server. Copy your public key. For example:
...
On the [Settings](https://radar.com/dashboard/settings) page, under *Auth tokens*, add a new public key. Paste the full contents of your public key PEM file, including the `BEGIN` and `END` lines. Copy the *Key ID*.

An auth token is a JSON Web Token (JWT). To create an auth token, create a JWT signed with your private key on your server. Pass `now` for `iat`, `now + ttl` for `exp`, `radar-api-v1` as `aud` (audience), a custom value for `iss` (issuer), your public key *Key ID* for `kid`, and any optional constraints as claims. For example:
...
const payload = {
  ...
  constraints: { // optional
    expectedCountryCode: constraints.expectedCountryCode,
    expectedStateCode: constraints.expectedStateCode,
...
Radar.initialize(
  authToken = authToken,
  options = RadarInitializeOptions(fraud = true)
)
```
