...
- `risk`: Risk information, including a risk `level` (none, low, medium, high, configurable with [User and device risk scores](#user-and-device-risk-scores)) and `reasons` (includes `proxy`, `compromised`, and `sharing` reasons from [Failure reasons](#failure-reasons)).
