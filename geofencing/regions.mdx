...
Regions supports country detection globally. Each country has a `name` and a unique 2-letter [ISO 3166](https://en.wikipedia.org/wiki/List_of_ISO_3166_country_codes) `code`. View the [full list of countries](/geofencing/regions/countries).
...
Regions supports state detection in the US and Canada only. Each state has a `name` and a 2-letter `code` unique to the residing `country`. View the [full list of states](/geofencing/regions/states).

## DMAs

Regions supports DMA (designated market area) detection in the US only. Each DMA has a `name` and a unique `code`. View the [full list of DMAs](/geofencing/regions/dmas). Or, learn more about [DMAs](https://en.wikipedia.org/wiki/Media_market).
