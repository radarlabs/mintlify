...
- `externalIds` (array, required): An array of user IDs to send the track request to. Limit to a maximum of 10 users per request.
...
curl "https://api.radar.io/v1/users/refresh" \
  ...
  -H "Content-Type: application/json" \
  -X POST \
  -d '{"externalIds": ["userId1", "userId2", "userId3"]}'
```
...
```json
{
  ...
  "sent": ["userId1", "userId2"],
  "failed": [{"externalId":"userId3","reason":"Rate limit exceeded"}]
}
```
