...
Every API response includes an `X-Radar-Request-ID` header with a unique identifier for the request.

**Sample response header**

```sh
X-Radar-Request-ID: 01890a5d-ac96-774b-bcce-b302099a8057
```

If you contact support about a specific API call, include the request ID. The request ID allows us to quickly find and debug the exact request.
...
- **`accuracy`** (number, required): The accuracy of the user's current latitude and longitude, in meters. A number greater than 0. Used when evaluating the [confidence](/geofencing/geofences#confidence-and-accuracy) of geofence events.
...
Each turn-by-turn step includes `maneuver`, a string describing the maneuver type (for example, `start`, `left`, or `destination`).
